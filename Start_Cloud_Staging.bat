...
set "CLOUD_URL=http://localhost:3000/"
...
powershell -NoProfile -Command "$listener=Get-NetTCPConnection -State Listen -LocalPort 3000 -ErrorAction SilentlyContinue | Select-Object -First 1; if($listener){$process=Get-Process -Id $listener.OwningProcess -ErrorAction SilentlyContinue; $name=if($process){$process.ProcessName}else{'unknown'}; Write-Host ('CLOUD STAGING was not started: port 3000 is already in use by ' + $name + ' (PID ' + $listener.OwningProcess + ').'); exit 1}"
if errorlevel 1 exit /b 1

start "" /b python "%ROOT%scripts\Start_Cloud_Staging.py" > "%LOG%" 2>&1
powershell -NoProfile -Command "$ready=$false; 1..40 | ForEach-Object { try { $r=Invoke-WebRequest -UseBasicParsing 'http://localhost:3000/'; if($r.StatusCode -eq 200 -and $r.Content -match 'fbvzqdqjqcbjopuinknw'){ $ready=$true; break } } catch {}; Start-Sleep -Milliseconds 250 }; if(-not $ready){exit 1}" >nul 2>&1
...
:not_ready
echo CLOUD STAGING did not become ready at http://localhost:3000/.
