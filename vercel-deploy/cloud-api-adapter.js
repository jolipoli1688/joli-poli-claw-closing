...
window.createClawCloudAdapter = function createClawCloudAdapter(config) {
  ...
  const outletStorageKey = `joli-poli-claw:${projectRef}:active-store`;
  const outletStorage = typeof localStorage === "undefined" ? null : localStorage;
  let activeStoreId = String(outletStorage?.getItem(outletStorageKey) || "").trim();
  const setActiveStoreId = value => {
    activeStoreId = String(value || "").trim();
    if (activeStoreId) outletStorage?.setItem(outletStorageKey, activeStoreId);
    else outletStorage?.removeItem(outletStorageKey);
  };
  ...
  return {
    ...
    async request(path, options = {}) {
      ...
      const response = await fetch(requestUrl, {
        ...options,
        headers: { ...(options.headers || {}), "Content-Type": "application/json", apikey: publishableKey, Authorization: `Bearer ${token}`, ...(activeStoreId ? { "x-claw-store-id": activeStoreId } : {}) },
      });
      ...
      if (path === "/api/bootstrap") setActiveStoreId(body?.cloud_context?.active_store?.id || "");
      return body;
    },
  };
};
