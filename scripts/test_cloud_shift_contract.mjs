...
assert.match(edge, /activeClosingForDate/, "Start Shift must resolve the same-store active draft");
...
assert.match(app, /\/api\/active-closing\?report_date=/, "client refresh must resolve the exact active draft before rendering Start Shift");
assert.match(app, /openClosingV2179DailyWorkflow\(active\.closing_id\)/, "client refresh must hydrate the active draft");
