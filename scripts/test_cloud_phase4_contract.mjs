...
assert.match(styles, /backdrop-filter:\s*blur\(28px\) saturate\(125%\)/, "cloud login card must use the premium white-glass blur treatment");
assert.match(styles, /width: min\(680px, calc\(100vw - 48px\)\)/, "cloud login card must use the 680px desktop width cap");
assert.match(styles, /height: 520px/, "cloud login card must use the balanced 520px desktop height");
assert.match(styles, /background: rgba\(255, 255, 255, \.72\)/, "cloud login card must use translucent white glass");
assert.match(runtime, /claw-login-art/, "cloud login must render the Figma-derived decorative asset layer");
assert.equal((runtime.match(/figma-login\/asset-\d+\.svg/g) || []).length, 8, "cloud login must use eight Figma-derived decorative assets");
...
assert.match(styles, /linear-gradient\(135deg, #fff 0%, #fafaf8 45%, #f7f7f4 100%\)/, "cloud login must use the premium light-neutral background");
assert.match(styles, /left: 90px/, "desktop login form must remain left-aligned inside the glass panel");
assert.match(styles, /width: 500px/, "desktop login form group must preserve the reference form width");
assert.match(styles, /height: 54px/, "login inputs and submit button must use the polished 54px control height");
assert.match(styles, /background: #111827/, "login submit button must use the professional charcoal treatment");
assert.match(styles, /border-radius: 14px/, "login controls must use the polished 14px radius");
