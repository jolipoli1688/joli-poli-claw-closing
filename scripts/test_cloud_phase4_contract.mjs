...
assert.match(styles, /backdrop-filter:\s*blur\(28px\) saturate\(120%\)/, "cloud login card must use the dark reference frosted-glass treatment");
assert.match(styles, /width: min\(690px, calc\(100vw - 48px\)\)/, "cloud login card must use the square 690px desktop width cap");
assert.match(styles, /height: 690px/, "cloud login card must use the square reference height");
assert.match(styles, /background: rgba\(100, 96, 145, \.42\)/, "cloud login card must use translucent dark purple glass");
...
assert.match(styles, /#2c2a52/, "cloud login must use the dark navy reference base");
assert.match(styles, /left: 95px/, "desktop login form must remain left-aligned inside the glass panel");
assert.match(styles, /width: 500px/, "desktop login form group must preserve the reference form width");
assert.match(styles, /height: 52px/, "login inputs and submit button must use the reference control height");
assert.match(styles, /background: linear-gradient\(90deg, #f6c800, #f4d500\)/, "login submit button must use the JOLI POLI yellow reference adaptation");
