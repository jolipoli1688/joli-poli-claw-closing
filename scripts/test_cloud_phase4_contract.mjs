...
assert.match(styles, /backdrop-filter:\s*blur\(34px\) saturate\(130%\)/, "cloud login card must use the refined frosted-glass treatment");
assert.match(styles, /width: min\(68vw, 1140px\)/, "cloud login card must use the final 1140px desktop width cap");
assert.match(styles, /height: clamp\(500px, 47\.22vh, 510px\)/, "cloud login card must use the final 510px desktop height");
assert.match(styles, /rgba\(255, 255, 255, \.50\)/, "cloud login card must be translucent light glass");
...
assert.equal((runtime.match(/stroke-width="(?:104|90)"/g) || []).length, 2, "cloud login must use exactly two restrained major tubes");
assert.match(styles, /path:nth-of-type\(2\)/, "upper login ribbon must remain separately positioned above the form");
assert.ok(!runtime.includes("claw-login-ribbon"), "cloud login must not retain rejected donut-ribbon elements");
assert.match(styles, /#fafafc/, "cloud login must retain the light premium base");
assert.match(runtime, /#f4d500/i, "cloud login artwork must use the JOLI POLI yellow palette");
assert.match(styles, /left: calc\(75% - 216px\)/, "desktop login form must remain centered in the right half of the glass panel");
assert.match(styles, /top: calc\(50% \+ 14px\)/, "desktop login stack must use the final vertical composition offset");
assert.match(styles, /width: 432px/, "desktop login form group must preserve the professional form width");
assert.match(styles, /height: 54px/, "login inputs and submit button must preserve the refined control height");
assert.match(styles, /background: #191825/, "login submit button must retain the dark reference action color");
