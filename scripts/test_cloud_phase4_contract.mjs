...
assert.match(launcher, /\("localhost", 3000\)/, "cloud host must use localhost:3000");
...
assert.match(manualLauncher, /http:\/\/localhost:3000\//, "manual launcher must open localhost:3000");
assert.match(manualLauncher, /Get-NetTCPConnection -State Listen -LocalPort 3000/, "manual launcher must refuse an occupied port 3000");
