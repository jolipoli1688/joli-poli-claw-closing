...
assert.equal(voidedRefill.refill.id, negativeRefillEvent.id, "void response must identify the voided event");
assert.equal(voidedRefill.product.refill_qty, 10, "void must recalculate the signed refill aggregate from non-voided events");
assert.equal(voidedRefill.product.qty_used, 12, "void must recalculate Qty Used from non-voided events");
assert.ok(!voidedRefill.product.refill_history.some(event => event.id === negativeRefillEvent.id), "voided event must no longer appear in active refill history");
const voidReload = await expectStatus(api(developer.access_token, `/api/closings/${refillPlus.closing_id}`), 200, "voided refill history reload");
const voidedRefillProduct = voidReload.products.find(product => product.Product_ID === meterProducts[0].product_id);
