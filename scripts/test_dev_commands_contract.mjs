...
assert.match(runner, /const stopChild = \(signal\)/, "launcher must own its child shutdown lifecycle");
assert.match(runner, /child\.kill\(signal\)/, "launcher must first signal only its direct child");
assert.match(runner, /taskkill/, "Windows launcher must have a bounded child-only cleanup fallback");
assert.match(runner, /"\/PID", String\(child\.pid\), "\/T", "\/F"/, "fallback must target only the spawned child process tree");
