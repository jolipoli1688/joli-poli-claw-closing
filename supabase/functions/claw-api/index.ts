...
function manager(ctx: Context) {
  return ["store_manager", "head_office", "admin"].includes(ctx.profile.role);
}
...
async function saveImage(ctx: Context, body: any) {
  if (!manager(ctx)) throw new Error("Store Manager role required for images.");
...
async function saveClosing(ctx: Context, payload: any) {
  ...
  if (workflow === "finalized" && (!manager(ctx) || !payload.verified_by)) throw new Error(!manager(ctx) ? "Only an authorized manager can finalize." : "Verified By is required before finalizing.");
...
async function serve(req: Request) {
  ...
  try {
    ...
    if (path === "/api/machines" && req.method === "POST") {
      if (!manager(ctx)) return fail("Store Manager role required.", 403);
    ...
    if (path === "/api/settings/unlock" && req.method === "POST") return manager(ctx) ? json({ ok: true }) : fail("Store Manager role required.", 403);
    ...
    if (path.startsWith("/api/closings/") && req.method === "DELETE") { if (!manager(ctx)) return fail("Store Manager role required.", 403); const id = path.split("/").pop()!; const result = await ctx.admin.from("daily_closings").update({ status: "void", voided_by: ctx.userId, voided_at: new Date().toISOString(), void_reason: String(body.reason || "") }).eq("id", id).eq("store_id", ctx.store.id).eq("status", "draft"); if (result.error) throw new Error(result.error.message); return json({ ok: true }); }
