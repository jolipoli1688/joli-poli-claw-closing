...
async function readMachines(ctx: Context, activeOnly = false) {
  ...
  return await Promise.all((data || []).map(async (machine: any) => {
    const products = await Promise.all((machine.machine_styles || []).filter((style: any) => !activeOnly || style.is_active).sort((a: any, b: any) => a.sort_order - b.sort_order).map(async (style: any) => {
      ...
      return { product_id: style.id, barcode: style.barcode, product_name: style.product_name || "", starting_qty: integer(style.starting_qty), image_file: style.image_path || "", image_url: signed.data?.signedUrl || "" };
    }));
...
async function serve(req: Request) {
  ...
  try {
    ...
    if (path === "/api/users" && req.method === "GET") { if (!developer(ctx)) return fail("Developer role required.", 403); const { data, error } = await ctx.admin.from("profiles").select("id,username,role,is_active,all_stores,user_store_access!user_store_access_user_id_fkey(store_id,stores(code,name))").order("username"); if (error) throw new Error(error.message); return json((data || []).map((row: any) => ({ id: row.id, username: row.username, role: row.role, status: row.is_active ? "active" : "inactive", outlets: row.all_stores ? [{ code: "ALL", name: "All outlets" }] : (row.user_store_access || []).map((access: any) => ({ code: access.stores?.code, name: access.stores?.name })) }))); }
