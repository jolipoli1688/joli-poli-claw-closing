...
async function readMachines(ctx: Context, activeOnly = false, includeInactiveStyles = false) {
  ...
  return await Promise.all((data || []).map(async (machine: any) => {
    const products = await Promise.all((machine.machine_styles || []).filter((style: any) => includeInactiveStyles || style.is_active).sort((a: any, b: any) => a.sort_order - b.sort_order).map(async (style: any) => {
      const signed = style.image_path ? await ctx.admin.storage.from(IMAGE_BUCKET).createSignedUrl(style.image_path, 300) : { data: null };
      return { product_id: style.id, barcode: style.barcode, product_name: style.product_name || "", starting_qty: integer(style.starting_qty), active: style.is_active, image_file: style.image_path || "", image_url: signed.data?.signedUrl || "" };
    }));
