...
async function serve(req: Request) {
  ...
  try {
    ...
    if (path === "/api/users" && req.method === "GET") { if (!developer(ctx)) return fail("Developer role required.", 403); const { data, error } = await ctx.admin.from("profiles").select("id,username,role,is_active,all_stores,user_store_access(store_id,stores(code,name))").order("username"); if (error) throw new Error(error.message); return json((data || []).map((row: any) => ({ id: row.id, username: row.username, role: row.role, status: row.is_active ? "active" : "inactive", outlets: (row.user_store_access || []).map((access: any) => ({ code: access.stores?.code, name: access.stores?.name })) }))); }
    if (path === "/api/users" && req.method === "POST") { if (!developer(ctx)) return fail("Developer role required.", 403); const username = normalizeUsername(body.username); const role = String(body.role || ""); if (!["developer", "admin", "outlet"].includes(role)) return fail("Role must be Developer, Admin, or Outlet.", 400); const password = String(body.temporary_password || ""); if (password.length < 12) return fail("Temporary password must be at least 12 characters.", 400); const outletCodes = Array.isArray(body.outlets) ? body.outlets.map(String) : []; if (role === "outlet" && !outletCodes.length) return fail("Outlet users need at least one outlet assignment.", 400); const existing = await ctx.admin.from("profiles").select("id").eq("username", username).maybeSingle(); if (existing.data) return fail("Username already exists.", 409); const created = await ctx.admin.auth.admin.createUser({ email: `${username}@claw.internal`, password, email_confirm: true, user_metadata: {} }); if (created.error || !created.data.user) throw new Error("Could not create the user."); const profile = await ctx.admin.from("profiles").update({ username, display_name: username, role, is_active: body.status !== "inactive", all_stores: role !== "outlet", updated_by: ctx.userId }).eq("id", created.data.user.id); if (profile.error) throw new Error(profile.error.message); if (outletCodes.length) { const { data: stores } = await ctx.admin.from("stores").select("id,code").in("code", outletCodes); if ((stores || []).length !== outletCodes.length) throw new Error("An assigned outlet does not exist."); const access = await ctx.admin.from("user_store_access").insert(stores!.map((store: any) => ({ user_id: created.data.user.id, store_id: store.id, granted_by: ctx.userId }))); if (access.error) throw new Error(access.error.message); } return json({ id: created.data.user.id, username, role, status: body.status === "inactive" ? "inactive" : "active" }, 201); }
    if (path.startsWith("/api/users/") && req.method === "PATCH") { if (!developer(ctx)) return fail("Developer role required.", 403); const userId = path.split("/").pop()!; if (body.username !== undefined) return fail("Username is immutable; create a controlled replacement user instead.", 400); const role = String(body.role || ""); if (role && !["developer", "admin", "outlet"].includes(role)) return fail("Role must be Developer, Admin, or Outlet.", 400); const update: any = { updated_by: ctx.userId }; if (role) update.role = role; if (body.status !== undefined) update.is_active = body.status !== "inactive"; if (role) update.all_stores = role !== "outlet"; const updated = await ctx.admin.from("profiles").update(update).eq("id", userId); if (updated.error) throw new Error(updated.error.message); return json({ ok: true }); }
    ...
    if (path === "/api/machines" && req.method === "POST") {
      ...
      if (machineId) {
        ...
        const created = await ctx.admin.from("machines").insert({ store_id: ctx.store.id, machine_type_id: machineType.data.id, machine_code: code, machine_number: integer(body.machine_number) || 1, display_name: String(body.machine_name || code), prize_category: String(body.prize_category || ""), is_active: body.active !== false, sort_order: integer(body.sort_order) || 1, notes: String(body.notes || "") }).select("*").single();
    ...
    if (path === "/api/new-closing") { const machines = await readMachines(ctx, true); return json({ report_date: url.searchParams.get("report_date") || new Date().toISOString().slice(0, 10), outlet: ctx.store.name, machines: machines.map((machine: any) => ({ machine_id: machine.Machine_ID, machine_name: machine.Machine_Name, machine_type: machine.Machine_Type, products: machine.Products.map((product: any) => ({ ...product, begin_qty: 0, refill_qty: 0, final_qty: 0 })), barcodes: machine.Barcodes, image_url: machine.Image_URL, capacity: 0, begin_prize: 0, refill_prize: 0, final_prize: 0, begin_coin_meter: 0, final_coin_meter: 0, status: "Working", notes: "" })) }); }
